...
async function walk(dir) {
  ...
  for (const entry of entries) {
    ...
    if (INPUT_EXTENSIONS.has(extname(entry.name).toLowerCase()))
      files.push(path);
  }
...
async function optimizeFile(file) {
  ...
  const outputPath = useWebp
    ? `${parse(file).dir}/${parse(file).name}.webp`
    : file;
...
for (const result of results) {
  if (result.status === "error") {
    console.error(
      ` error ${result.file.replace(PUBLIC_DIR + "/", "")} / ${result.error}`,
    );
    continue;
  }
